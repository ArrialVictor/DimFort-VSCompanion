...
|---|---|---|
| **Startup / pre-handshake** | LSP process dies before reaching `Running` (executable not on PATH, missing `[lsp]` extra, immediate Python crash) | **Canonical surface**: `installServerExitSurfacing`'s `Starting → Stopped` branch. Fires synchronously from the library's `$state = StartFailed` assignment, so it doesn't depend on the `_onStart` promise plumbing — which the library breaks across the close-then-cleanup sequence (sets `_onStart = undefined` before the original `start()`'s catch can run). **Belt-and-suspenders**: `client.start().catch(reportStartFailure)` in activate + try/catch wrapping in `doRebuildClient`, deduped against the state listener via a shared `state:Starting->Stopped` key. Previously `void client.start()` swallowed the rejection entirely. |
