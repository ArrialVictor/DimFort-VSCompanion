...
|---|---|---|
| `vscode.window.showErrorMessage` (popup, requires dismissal) | `reportStartFailure`, `installServerExitSurfacing`, `stats.checkWorkspace` wire-error branch, existing `dimfort.restartLanguageServer` failure handler. | "User must act on this" — the LSP can't function until they install `dimfort[lsp]`, restart, or otherwise unblock. Popup is the only surface that doesn't get overwritten by another component's `setStatusBarMessage` in the same tick. |
