...
| `DimFort: Status` | Pop a modal dialog summarising current DimFort settings + LSP client state; also tees the same text to `Output → DimFort Status` for later debugging (`dimfort.status`). |
